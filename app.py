...
def register():
    if request.method == 'GET':
        ...
        # CHANGED: Added robust validation for username, email, and password
        ...
        # CHANGED: Optimized database query to check for existing username or email
        ...
        try:
            # CHANGED: Hash the password before storing it
            hashed_password = generate_password_hash(password)
            user = Users(username=username, email=email, password=hashed_password, roles=roles)
            db.session.add(user)
            db.session.commit()
            
            # CHANGED: Simplified session management
            session['user_id'] = user.id
            session['user_role'] = user.roles
            
            flash('Sign Up Successful')  # CHANGED: Flash message for successful registration
            return redirect(url_for('dashboard'))
        
        except Exception as e:
            # CHANGED: Added error handling for database issues
            db.session.rollback()
...
def login():
    if request.method == 'GET':
        ...
        username = request.form['username'].strip().lower()  # Convert to lowercase and remove extra spaces
        password = request.form['password']
        ...
        if user and check_password_hash(user.password, password):  # Secure password check
            ...
            session['is_admin'] = getattr(user, 'is_admin', False)  # Prevent KeyError
            
            flash("Successfully Logged In", "success")
            return redirect(url_for('dashboard'))
        else:
            flash("Invalid username or password", "danger")  # Use flash messages instead of passing errors
...
def new_assignments(course_id):
    ...
    if request.method == 'POST':
        ...
        due_str = request.form['end_date']  # String format: 'YYYY-MM-DD'
        ...
        assignment = Assignment(
            title=name,
            description=desc,
            due_date=due_date,  # Use the converted date object
            course_id=course_id,
            user_id=user_id
        )
...
def attendance_stats(course_id):
    ...
    if session.get('is_admin', False) == False:
